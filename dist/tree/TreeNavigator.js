...
class TreeNavigator {
    ...
    getSummary() {
        ...
        // Build text summary
        const lines = [];
        lines.push('Execution completed in ' + totalDuration.toFixed(0) + 'ms across ' +
        ...
        if (slowNodes.length > 0) {
            const slowParts = slowNodes.map((n) => '[' + n.id + '] ' + (n.durationMs ?? 0).toFixed(0) + 'ms');
    ...
    drillDown(id) {
        ...
        const reads = stageData?.reads ?? [];
        const writes = stageData?.writes ?? [];
        const errors = stageData?.errors ?? [];
        ...
        if (reads.length > 0) {
            ...
            for (const read of reads) {
                lines.push('  - ' + read.path.join('.') + '.' + read.key + ' = ' + read.valueSummary);
            }
        }
        if (writes.length > 0) {
            lines.push('');
            ...
            for (const write of writes) {
                ...
                lines.push('  - ' + write.path.join('.') + '.' + write.key + ' = ' + write.valueSummary + ' ' + op);
            }
        }
    ...
    getChildren(id) {
        ...
        const lines = summaries.map((s) => '[' + s.id + '] ' + s.name + ' — ' + s.description);
        return {
            nodes: summaries,
    ...
    getErrors() {
        const errorNodes = this.tree.getErrorNodes();
        const summaries = errorNodes.map((node) => ({
            id: node.id,
            name: node.name,
     ...
     */
    getSlowStages(thresholdMs = 1000) {
        ...
        const lines = summaries.map((s) => '[' + s.id + '] ' + s.name + ' — ' + (s.durationMs ?? 0).toFixed(0) + 'ms');
        return {
            nodes: summaries,
            totalResults: summaries.length,
            textResult: summaries.length > 0
